...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <Link to="/"><img className="h-9 w-auto" src="/new-logo.png" alt="logo" width={130} height={38} /></Link>
                    ...
                    {isLoggedIn ? (
                        <div className="relative group">
                            ...
                            <div className="absolute hidden group-hover:block top-6 right-0 pt-4">
                                <button onClick={() => logout()} className="bg-white/10 border-2 border-white/10 px-5 py-1.5 rounded">
                                    Logout
                                </button>
                            </div>
                        </div>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                {
                    isLoggedIn ? <Link onClick={() => setIsOpen(false)} to="/my-generation" className="hover:text-pink-500 transition">My Generations</Link> :
                        <Link onClick={() => setIsOpen(false)} to="#" className="hover:text-pink-500 transition">About</Link>
                }
